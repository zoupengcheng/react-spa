...
class SetupWizard extends Component {
    ...
    _checkOldPassword = (rule, value, callback) => {
        const { formatMessage } = this.props.intl
        const currentUserId = localStorage.adminId
    ...
    _checkOutboundName = (rule, value, callback) => {
        ...
        if (value && _.indexOf(this.state.outboundNameList, value) > -1) {
            callback(formatMessage({id: "LANG2137"}))
        } else if (value && _.indexOf(tmpNameList, value) > -1) {
            callback(formatMessage({id: "LANG2137"}))
        } else {
        ...
        $.ajax({
            ...
            success: function(res) {
                ...
                if (bool) {
                    ...
                    if (timezoneItem.self_defined_time_zone === 'customize') {
                        customizeDisable = false
                    } else {
                        customizeDisable = true
                    }
    ...
    _handleSubmit = () => {
        ...
        this.props.form.validateFieldsAndScroll({force: true}, (err, values) => {
            if ((!err) || (err && (err.hasOwnProperty('outbound_rt_name') || err.hasOwnProperty('trunk_name')))) {
                ...
                action.user_id = localStorage.adminId
                /* password */
                        ...
                        <Form className={ wizardItem[this.state.current] === 'extensionItem' ? 'display-block' : 'hidden'}>
                            ...
                            <FormItem
                                ...
                                { getFieldDecorator('batch_extension', {
                                    rules: [{
                                        validator: wizardItem[this.state.current] === 'extensionItem' ? this._checkExtenRequire : ''
                                    }, {
                                        validator: (data, value, callback) => {
                                            Validator.digits(data, value, callback, formatMessage)
                                        }
                                    ...
                                    <Input onChange={this._onInputExtenChange} maxLength='18' />
                                ) }
                                </Popover>
                            </FormItem>
                            <FormItem
                            ...
                            <FormItem
                                ...
                                { getFieldDecorator('batch_password', {
                                    ...
                                    <RadioGroup onChange={ this._onChangeRadioSecret }>
