...
import { Form, Input, Button, Row, Col, Checkbox, message, Modal, Popover, Select, Table, Tabs, Popconfirm } from 'antd'
...
class GlobalTemplates extends Component {
    ...
    _checkReady = () => {
        ...
        if (ZEROCONFIG.isDataReady() === 1) {
            ...
            ZEROCONFIG.connector.checkZeroConfigInvalidModels($('#invalidModelWarningGT').html(), true)
        } else {
    ...
    _editGlobalTemplate = (record) => {
        browserHistory.push('/value-added-features/zeroConfig/editGlobalTemplate/' + record.id + '/' + record.name)
    }
